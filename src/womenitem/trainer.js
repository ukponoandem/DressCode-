...
import Womenteam from "../womenterm/womenteam";
...
const Trainer = () => {
  ...
  const bootsProducts = Womenteam.filter(item => item.category === "trainer");
  ...
  return (
    <div>
      ...
      <h2 className="header-text">Women's Sandals</h2>
  ...
  <span aria-hidden="" className="marquee" onClick={() => alert("Order placed successfully!")}>Placed Order!</span>
                ...
                <p>09161248397</p>
                {!buttonClicked && (
                  <button onClick={handleAddToCart} className="cart-buuton">ADD TO CART</button>
                )}
